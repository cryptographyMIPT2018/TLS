...
class Signer:
    ...
    def sign(self, m):
        ...
        while True:
            while True:
                # step 3
                k = random.randint(1, self.curve.q - 1)

                # step 4
                P = self.curve.get_forming()
                C = self.curve.multiply_by_number(P, k)
                r = self.get_point_x(C) % self.curve.q
        ...
        # step 6
        sv = s.to_bytes(256, byteorder='little')
        rv = r.to_bytes(256, byteorder='little')
    ...
    def check(self, m, signature):
        ...
        if len(signature) != 512:
            return False
        s = int.from_bytes(signature[:256], byteorder='little', signed=False)
        r = int.from_bytes(signature[256:], byteorder='little', signed=False)
        if not 0 < s < self.curve.q or not 0 < r < self.curve.q:
            return False
        ...
        # step 4
        v = pow(e, self.curve.q - 2, self.curve.q)

        # step 5
        z1 = (s * v) % self.curve.q
        z2 = (self.curve.q - (r * v) % self.curve.q) % self.curve.q
